...
from operators import div, grad, rot
from unet_parts import *


def get_Net(params):
	if params.net != "Fluid_model":
		raise ValueError(f"unsupported network '{params.net}'. Only 'Fluid_model' is available in this fluid-only codebase.")
	return fluid_model(
...
class fluid_model(nn.Module):
	...
	def forward(self, hidden_state, v_cond, v_mask):
		...
		# subtract mean of a_z and p
		out[:, 0:1, :, :] = out[:, 0:1, :, :] - torch.mean(out[:, 0:1, :, :], dim=(2, 3)).unsqueeze(2).unsqueeze(3)
		out[:, self.v_size : (self.v_size + 1), :, :] = out[:, self.v_size : (self.v_size + 1), :, :] - torch.mean(
			out[:, self.v_size : (self.v_size + 1), :, :], dim=(2, 3)
...
# 4th order splines
def p4_1(offsets):
	return (offsets - 1) ** 4 * (1 + 4 * offsets + 10 * offsets**2 + 20 * offsets**3) * heaviside(offsets) + (
		-offsets - 1
	) ** 4 * (1 - 4 * offsets + 10 * offsets**2 - 20 * offsets**3) * heaviside(-offsets)


def p4_2(offsets):
	return (
		(offsets - 1) ** 4 * (1 * offsets + 4 * offsets**2 + 10 * offsets**3) * heaviside(offsets)
		+ (-offsets - 1) ** 4 * (1 * offsets - 4 * offsets**2 + 10 * offsets**3) * heaviside(-offsets)
	) * 4
...
def p4_3(offsets):
	return (
		(offsets - 1) ** 4 * (0.5 * offsets**2 + 2 * offsets**3) * heaviside(offsets)
		+ (-offsets - 1) ** 4 * (0.5 * offsets**2 - 2 * offsets**3) * heaviside(-offsets)
	) * 32
...
def save_buffers():
	...
	with open(path, "wb") as handle:
		pickle.dump(
			{
				"vel": kernel_buffer_velocity,
				"vel_superres": kernel_buffer_velocity_superres,
				"pres": kernel_buffer_pressure,
				"pres_superres": kernel_buffer_pressure_superres,
			},
			handle,
		)
...
def load_buffers():
	global kernel_buffer_velocity, kernel_buffer_velocity_superres, kernel_buffer_pressure, kernel_buffer_pressure_superres
	path = "Logger/buffers/buffers.dic"
	with open(path, "rb") as handle:
		buffers = pickle.load(handle)
		kernel_buffer_velocity = buffers.get("vel", {})
		kernel_buffer_velocity_superres = buffers.get("vel_superres", {})
		kernel_buffer_pressure = buffers.get("pres", {})
		kernel_buffer_pressure_superres = buffers.get("pres_superres", {})
...
def interpolate_states(old_hidden_states, new_hidden_states, offset, dt=1, orders_v=[2, 2], orders_p=[0, 0]):
	...
	:return: interpolated fields for a_z, v, dv_dt, grad_v, laplace_v, p, grad_p
	"""
...
def superres_states(old_hidden_states, new_hidden_states, offset, dt=1, orders_v=[2, 2], orders_p=[0, 0], resolution_factor=1):
	"""
	:return: superresolution fields for a_z, v, dv_dt, grad_v, laplace_v, p, grad_p
	"""
	v_size = np.prod([i + 1 for i in orders_v])
...
def superres_2d_velocity(weights, orders=[2, 2], resolution_factor=1):
	...
	if res_key in kernel_buffer_velocity_superres:
...
def interpolate_2d_pressure(weights, offsets, orders=[0, 0]):
	...
	"""
	offset_key = f"{offsets[0]} {offsets[1]}, orders: {orders}"
	if offset_key in kernel_buffer_pressure:
...
def superres_2d_pressure(weights, orders=[0, 0], resolution_factor=1):
	res_key = f"{resolution_factor}, orders: {orders}"
	if res_key in kernel_buffer_pressure_superres:
