...
def load_images():
	...
	if IMAGE_CACHE is None:
		...
		image_paths = {
			"cyber": "imgs/cyber.png",
			"fish": "imgs/fish.png",
			"smiley": "imgs/smiley.png",
			"wing": "imgs/wing_profile.png",
		}
		IMAGE_CACHE = {}
		for name, path in image_paths.items():
			image = torch.tensor(np.asarray(Image.open(path))).float()
			IMAGE_CACHE[name] = (torch.mean(image, dim=2) < 100).float()
	return IMAGE_CACHE
